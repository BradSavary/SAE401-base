...
class PostService
{
    ...
    public function create(CreatePostPayload $payload): void
    {
        ...
        // Gestion de l'upload du fichier média
        if ($payload->getMedia() instanceof UploadedFile) {
            $uploadDir = 'uploads/media'; // Répertoire d'upload
            $fileName = uniqid() . '.' . $payload->getMedia()->guessExtension();
            $payload->getMedia()->move($uploadDir, $fileName);
    ...
    public function formatPostDetails(Post $post, ?User $currentUser, string $baseUrl, string $uploadDir): array
    {
        ...
        $mediaUrl = $post->getMedia() ? $baseUrl . '/uploads/media/' . $post->getMedia() : null;    
        ...
        $postDetails = [
            ...
            'media' => $mediaUrl, // Ajout de l'URL du média
        ];
